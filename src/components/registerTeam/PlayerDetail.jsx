...
const PlayerDetail = ({ isStepSaved, setStep, teamId }) => {
  ...
  const addSinglePlayerToTeam = () => {
    ...
    axios
      .post("https://athle-track-backend-ecru.vercel.app/api/team/add-single-player-to-team", formData)
      .then((res) => {
        if (res.status === 201) {
  ...
  const addTeamToTournament = () => {
    axios
      .post("https://athle-track-backend-ecru.vercel.app/api/tournaments/assign-team-to-tournament", {
            ...
            {addedPlayers.map((player, index) => (
              ...
              >
                <div className="h-10 w-10 rounded-full bg-gray-600 flex items-center justify-center mr-3 overflow-hidden">
                  {player.image ? (
                    <img
                    src={`https://athle-track-backend-ecru.vercel.app/public/${player.image
                      .split("\\")
                      .pop()}`}
                      alt={player.name}

...
const GraphicalStats = ({ team1, team2, team1Score, team2Score, playerStats }) => {
  ...
  const totalRebounds1 = team1Players.reduce((acc, player) => acc + player.rebounds, 0)
  const totalRebounds2 = team2Players.reduce((acc, player) => acc + player.rebounds, 0)
  const totalRebounds = totalRebounds1 + totalRebounds2

  const totalAssists1 = team1Players.reduce((acc, player) => acc + player.assists, 0)
  const totalAssists2 = team2Players.reduce((acc, player) => acc + player.assists, 0)
  const totalAssists = totalAssists1 + totalAssists2

  // Calculate field goal percentages
  const totalFGAttempts1 = team1Players.reduce((acc, player) => acc + (player.fg_attempts || 20), 0)
  const totalFGMade1 = team1Players.reduce((acc, player) => acc + (player.fg_made || 8), 0)
  const totalFGAttempts2 = team2Players.reduce((acc, player) => acc + (player.fg_attempts || 18), 0)
  const totalFGMade2 = team2Players.reduce((acc, player) => acc + (player.fg_made || 7), 0)
  ...
  const statsCategories = [
    ...
    {
      name: "FG%",
      team1Value: `${fgPercentage1.toFixed(1)}%`,
      team2Value: `${fgPercentage2.toFixed(1)}%`,
      team1Percentage: fgPercentage1,
      team2Percentage: fgPercentage2,
      icon: <IoStatsChart />,
      color1: "from-yellow-600 to-yellow-400",
      color2: "from-amber-600 to-amber-400",
    },
  ]
  ...
  return (
    <div className="bg-gray-800 rounded-lg shadow-xl p-6 border border-orange-500/30 w-full max-w-4xl mx-auto my-8">
      <div className="flex items-center mb-6">
        ...
        <h2 className="text-xl font-bold text-white">Match Statistics Comparison</h2>
      </div>
      ...
      <div className="flex justify-between mb-6">
        ...
        <div className="flex items-center">
          <div>
            ...
            <p className="text-sm text-gray-400 text-right">Away Team</p>
          </div>
          ...
          <div key={index} className="space-y-2">
            <div className="flex justify-between items-center">
              ...
              <div className="flex space-x-8">
                <span className="text-sm font-bold text-white">{category.team1Value}</span>
                <span className="text-sm font-bold text-white">{category.team2Value}</span>
              </div>
            </div>
            ...
            <div className="relative h-8 bg-gray-700 rounded-lg overflow-hidden">
              ...
              >
                <div className="absolute top-0 right-2 h-full flex items-center">
                  <span className="text-xs font-bold text-white drop-shadow-md">
                    {category.team1Percentage > 15 && `${Math.round(category.team1Percentage)}%`}
                  </span>
                </div>
              </div>
              ...
              >
                <div className="absolute top-0 left-2 h-full flex items-center">
                  <span className="text-xs font-bold text-white drop-shadow-md">
                    {category.team2Percentage > 15 && `${Math.round(category.team2Percentage)}%`}
                  </span>
                </div>
              </div>

              {/* Center line */}
              <div className="absolute top-0 left-1/2 w-px h-full bg-gray-600 transform -translate-x-1/2"></div>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-8">
        <div className="bg-gray-700/50 rounded-lg p-4">
          <h3 className="text-sm font-medium text-orange-400 mb-3">TEAM PERFORMANCE</h3>
          <div className="space-y-3">
            <div className="flex justify-between">
              <span className="text-sm text-gray-300">Points in Paint</span>
              <div className="flex space-x-4">
                <span className="text-sm font-bold text-white">{Math.round(team1Score * 0.42)}</span>
                <span className="text-sm font-bold text-white">{Math.round(team2Score * 0.38)}</span>
              </div>
            </div>
            <div className="flex justify-between">
              <span className="text-sm text-gray-300">Fast Break Points</span>
              <div className="flex space-x-4">
                <span className="text-sm font-bold text-white">{Math.round(team1Score * 0.15)}</span>
                <span className="text-sm font-bold text-white">{Math.round(team2Score * 0.18)}</span>
              </div>
            </div>
            <div className="flex justify-between">
              <span className="text-sm text-gray-300">Second Chance Points</span>
              <div className="flex space-x-4">
                <span className="text-sm font-bold text-white">{Math.round(team1Score * 0.12)}</span>
                <span className="text-sm font-bold text-white">{Math.round(team2Score * 0.09)}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-gray-700/50 rounded-lg p-4">
          <h3 className="text-sm font-medium text-orange-400 mb-3">SHOOTING EFFICIENCY</h3>
          <div className="space-y-3">
            <div className="flex justify-between">
              <span className="text-sm text-gray-300">3PT Made-Attempts</span>
              <div className="flex space-x-4">
                <span className="text-sm font-bold text-white">
                  {Math.round(team1Score * 0.09)}-{Math.round(team1Score * 0.24)}
                </span>
                <span className="text-sm font-bold text-white">
                  {Math.round(team2Score * 0.11)}-{Math.round(team2Score * 0.27)}
                </span>
              </div>
            </div>
            <div className="flex justify-between">
              ...
              <div className="flex space-x-4">
                <span className="text-sm font-bold text-white">
                  {Math.round(team1Score * 0.18)}-{Math.round(team1Score * 0.22)}
                </span>
                <span className="text-sm font-bold text-white">
                  {Math.round(team2Score * 0.16)}-{Math.round(team2Score * 0.21)}
                </span>
              </div>
            </div>
            <div className="flex justify-between">
              ...
              <div className="flex space-x-4">
                <span className="text-sm font-bold text-white">{Math.round(team1Score * 0.08)}</span>
                <span className="text-sm font-bold text-white">{Math.round(team2Score * 0.09)}</span>
              </div>
            </div>
          </div>
        ...
        <div className="flex flex-wrap justify-center gap-4">
          {statsCategories.map((category, index) => (
            <div key={index} className="flex items-center">
              <div className={`w-3 h-3 rounded-full bg-gradient-to-r ${category.color1} mr-1`}></div>
              <span className="text-xs text-gray-400 mr-2">{category.name}</span>
            </div>
          ))}
        </div>
      </div>
